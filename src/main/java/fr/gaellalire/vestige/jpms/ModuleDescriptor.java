 ...
 * {@link #exports() exported-packages}, the services that they {@link #uses() use} or {@link #provides() provide}, and other components. <em>Normal</em> modules may
 * {@link #opens() open} specific packages. The module descriptor for an open modules does not declare any open packages (its {@code opens} method returns an empty set) but when
 * instantiated in the Java virtual machine then it is treated as if all packages are open. The module descriptor for an automatic module does not declare any dependences (except
 * for the mandatory dependency on {@code
 * java.base}), and does not declare any exported or open packages. Automatic module receive special treatment during resolution so that they read all other modules in the
 * configuration. When an automatic module is instantiated in the Java virtual machine then it reads every unnamed module and is treated as if all packages are exported and open.
 * </p>
 * <p>
...
public class ModuleDescriptor implements Comparable<ModuleDescriptor> {
    ...
    public static enum Modifier {
        /**
         * An open module. An open module does not declare any open packages but the resulting module is treated as if all packages are open.
         */
        OPEN,

        /**
         * An automatic module. An automatic module is treated as if it exports and opens all packages.
         * @apiNote This modifier does not correspond to a module flag in the binary form of a module declaration ({@code module-info.class}).
         */
        AUTOMATIC,
    ...
    public final static class Requires implements Comparable<Requires> {
        ...
        public static enum Modifier {
            ...
            /**
             * The dependence causes any module which depends on the <i>current module</i> to have an implicitly declared dependence on the module named by the {@code Requires}.
             */
            TRANSITIVE,
         ...
         * @return The version of the module if recorded at compile-time, or an empty {@code Optional} if no version was recorded or the version string recorded is
         *         {@linkplain Version#parse(String) unparseable}
         */
        public Version compiledVersion() {
            return (compiledVersion);
        }

        /**
         * Returns the string with the possibly-unparseable version of the module if recorded at compile-time.
         * @return The string containing the version of the module if recorded at compile-time, or an empty {@code Optional} if no version was recorded
         * @see #compiledVersion()
         */
         ...
         * <p>
         * Two {@code Requires} objects are compared by comparing their module names lexicographically. Where the module names are equal then the sets of modifiers are compared in
         * the same way that module modifiers are compared (see {@link ModuleDescriptor#compareTo ModuleDescriptor.compareTo}). Where the module names are equal and the set of
         * modifiers are equal then the version of the modules recorded at compile-time are compared. When comparing the versions recorded at compile-time then a dependence that
         * has a recorded version is considered to succeed a dependence that does not have a recorded version. If both recorded versions are {@linkplain Version#parse(String)
         * unparseable} then the {@linkplain #rawCompiledVersion() raw version strings} are compared lexicographically.
         * </p>
         * @param that The module dependence to compare
         ...
         * <p>
         * If the given object is not a {@code Requires} then this method returns {@code false}. Two module dependence objects are equal if the module names are equal, set of
         * modifiers are equal, and the compiled version of both modules is equal or not recorded for both modules.
         * </p>
         * <p>
         ...
         * <p>
         * The hash code is based upon the module name, modifiers, and the module version if recorded at compile time. It satisfies the general contract of the
    ...
    public final static class Exports implements Comparable<Exports> {
         ...
         * @return The set of target module names or for an unqualified export, an empty set
         */
         ...
         * <p>
         * Two {@code Exports} objects are compared by comparing the package names lexicographically. Where the packages names are equal then the sets of modifiers are compared in
         * the same way that module modifiers are compared (see {@link ModuleDescriptor#compareTo ModuleDescriptor.compareTo}). Where the package names are equal and the set of
         * modifiers are equal then the set of target modules are compared. This is done by sorting the names of the target modules in ascending order, and according to their
         * natural ordering, and then comparing the corresponding elements lexicographically. Where the sets differ in size, and the larger set contains all elements of the smaller
         * set, then the larger set is considered to succeed the smaller set.
         * </p>
         * @param that The module export to compare
         ...
         * <p>
         * The hash code is based upon the modifiers, the package name, and for a qualified export, the set of modules names to which the package is exported. It satisfies the
         ...
         * <p>
         * If the given object is not an {@code Exports} then this method returns {@code false}. Two module exports objects are equal if their set of modifiers is equal, the
     ...
     * </p>
     * <p>
     * The <em>opens</em> directive in a module declaration declares a package to be open to allow all types in the package, and all their members, not just public types and their
     * public members to be reflected on by APIs that support private access or a way to bypass or suppress default Java language access control checks.
     * </p>
    ...
    public final static class Opens implements Comparable<Opens> {
        ...
        /**
         * For a qualified opens, returns the non-empty and immutable set of the module names to which the package is open. For an unqualified opens, returns an empty set.
         * @return The set of target module names or for an unqualified opens, an empty set
         */
        public Set<String> targets() {
            return targets;
        }
         ...
         * <p>
         * Two {@code Opens} objects are compared by comparing the package names lexicographically. Where the packages names are equal then the sets of modifiers are compared in
         * the same way that module modifiers are compared (see {@link ModuleDescriptor#compareTo ModuleDescriptor.compareTo}). Where the package names are equal and the set of
         * modifiers are equal then the set of target modules are compared. This is done by sorting the names of the target modules in ascending order, and according to their
         * natural ordering, and then comparing the corresponding elements lexicographically. Where the sets differ in size, and the larger set contains all elements of the smaller
         * set, then the larger set is considered to succeed the smaller set.
         * </p>
         * @param that The module opens to compare
         ...
         * <p>
         * The hash code is based upon the modifiers, the package name, and for a qualified opens, the set of modules names to which the package is opened. It satisfies the general
         ...
         * <p>
         * If the given object is not an {@code Opens} then this method returns {@code false}. Two {@code Opens} objects are equal if their set of modifiers is equal, the package
         * names are equal and the set of target module names is equal.
         * </p>
         * <p>
         * This method satisfies the general contract of the {@link java.lang.Object#equals(Object) Object.equals} method.
         * </p>
    ...
    public final static class Provides implements Comparable<Provides> {
         ...
         * @return A non-empty and unmodifiable list of the fully qualified class names of the providers or provider factories
         */
         ...
         * <p>
         * Two {@code Provides} objects are compared by comparing the fully qualified class name of the service type lexicographically. Where the class names are equal then the
         * list of the provider class names are compared by comparing the corresponding elements of both lists lexicographically and in sequence. Where the lists differ in size,
         * {@code N} is the size of the shorter list, and the first {@code N} corresponding elements are equal, then the longer list is considered to succeed the shorter list.
         * </p>
         ...
         * <p>
         * If the given object is not a {@code Provides} then this method returns {@code false}. Two {@code Provides} objects are equal if the service type is equal and the list of
         * providers is equal.
         * </p>
         * <p>
         * This method satisfies the general contract of the {@link java.lang.Object#equals(Object) Object.equals} method.
     ...
     * <p>
     * A version string has three components: The version number itself, an optional pre-release version, and an optional build version. Each component is a sequence of tokens;
     * each token is either a non-negative integer or a string. Tokens are separated by the punctuation characters {@code '.'}, {@code '-'}, or {@code '+'}, or by transitions from
     * a sequence of digits to a sequence of characters that are neither digits nor punctuation characters, or vice versa.
     * <ul>
     * <li>The <i>version number</i> is a sequence of tokens separated by {@code '.'} characters, terminated by the first {@code '-'} or {@code
     *   '+'} character.</li>
     * <li>The <i>pre-release version</i> is a sequence of tokens separated by {@code '.'} or {@code '-'} characters, terminated by the first {@code '+'} character.</li>
     * <li>The <i>build version</i> is a sequence of tokens separated by {@code '.'}, {@code '-'}, or {@code '+'} characters.
     * </ul>
     * <p>
     * When comparing two version strings, the elements of their corresponding components are compared in pointwise fashion. If one component is longer than the other, but
     * otherwise equal to it, then the first component is considered the greater of the two; otherwise, if two corresponding elements are integers then they are compared as such;
     * otherwise, at least one of the elements is a string, so the other is converted into a string if it is an integer and the two are compared lexicographically. Trailing integer
     * elements with the value zero are ignored.
     * <p>
     * Given two version strings, if their version numbers differ then the result of comparing them is the result of comparing their version numbers; otherwise, if one of them has
     * a pre-release version but the other does not then the first is considered to precede the second, otherwise the result of comparing them is the result of comparing their
     * pre-release versions; otherwise, the result of comparing them is the result of comparing their build versions.
    ...
    public final static class Version implements Comparable<Version> {
        ...
        private Version(String v) {
            ...
            List<Object> build = new ArrayList<Object>(2);
     ...
     * </p>
     * <p>
     * The set includes a dependency on "{@code java.base}" when this module is not named "{@code java.base}". If this module is an automatic module then it does not have a
     * dependency on any module other than "{@code java.base}".
     * </p>
     ...
     * </p>
     * @return A possibly-empty unmodifiable set of the fully qualified class names of the service types used
     */
     ...
     * </p>
     * @return The possibly-empty unmodifiable set of the services that this module provides
     */
     ...
     * </p>
     * @return This module's version, or an empty {@code Optional} if the module does not have a version or the version is {@linkplain Version#parse(String) unparseable}
     */
     ...
     * </p>
     * @return The string containing the version of the module or an empty {@code Optional} if the module does not have a version
     * @see #version()
     */
     ...
     * <p>
     * {@code ModuleDescriptor} defines the {@link #newModule newModule}, {@link #newOpenModule newOpenModule}, and {@link #newAutomaticModule newAutomaticModule} methods to create
     * builders for building <em>normal</em>, open, and automatic modules.
     * </p>
     * <p>
     * The set of packages in the module are accumulated by the {@code
     * Builder} as the {@link ModuleDescriptor.Builder#exports(String) exports}, {@link ModuleDescriptor.Builder#opens(String) opens}, {@link ModuleDescriptor.Builder#packages(Set)
     * packages}, {@link ModuleDescriptor.Builder#provides(String,List) provides}, and {@link ModuleDescriptor.Builder#mainClass(String) mainClass} methods are invoked.
     * </p>
     * <p>
     * The module names, package names, and class names that are parameters specified to the builder methods are the module names, package names, and qualified names of classes (in
     * named packages) as defined in the <cite>The Java&trade; Language Specification</cite>.
     * </p>
     * <p>
     * Example usage:
     * </p>
     ...
     * {
     *     &#64;code
     *     ModuleDescriptor descriptor = ModuleDescriptor.newModule("stats.core").requires("java.base").exports("org.acme.stats.core.clustering")
     *             .exports("org.acme.stats.core.regression").packages(Set.of("org.acme.stats.core.internal")).build();
     * }
     * </pre>
     *
     * @apiNote A {@code Builder} checks the components and invariants as components are added to the builder. The rationale for this is to detect errors as early as possible and
     *          not defer all validation to the {@link #build build} method.
     * @since 9
     * @spec JPMS
     */
    public static final class Builder {
        ...
        /**
         * Adds a dependence on a module with the given (and possibly empty) set of modifiers. The dependence includes the version of the module that that was recorded at
         * compile-time.
         * @param ms The set of modifiers
         * @param mn The module name
         * @param compiledVersion The version of the module recorded at compile-time
         * @return This builder
         * @throws IllegalArgumentException If the module name is {@code null}, is not a legal module name, or is equal to the module name that this builder was initialized to
         *         build
         * @throws IllegalStateException If the dependence on the module has already been declared or this builder is for an automatic module
         */
         ...
         * @return This builder
         * @throws IllegalArgumentException If the package name is {@code null} or is not a legal package name, the set of target modules is empty, or the set of target modules
         ...
         * @param obj The {@code Opens} object
         * @return This builder
        ...
        /**
         * Provides a service with one or more implementations. The package for each {@link Provides#providers provider} (or provider factory) is added to the module if not already
         * added.
         * @param p The provides
         * @return This builder
         ...
         * @param service The service type
         * @param providers The list of provider or provider factory class names
         * @return This builder
         * @throws IllegalArgumentException If the service type or any of the provider class names is {@code null} or not a qualified name of a class in a named package, or the
         ...
         * @param pns The (possibly empty) set of package names
         * @return This builder
         ...
         * @param vs The version string to parse
         * @return This builder
        ...
        /**
         * Sets the module main class. The package for the main class is added to the module if not already added. In other words, this method is equivalent to first invoking this
         * builder's {@link #packages(Set) packages} method to add the package name of the main class.
         * @param mc The module main class
         * @return This builder
         ...
         * <p>
         * The module will require "{@code java.base}" even if the dependence has not been declared (the exception is when building a module named "{@code java.base}" as it cannot
         * require itself). The dependence on "{@code java.base}" will have the {@link java.lang.module.ModuleDescriptor.Requires.Modifier#MANDATED MANDATED} modifier if the
         * dependence was not declared.
         * </p>
         * @return The module descriptor
         */
        public ModuleDescriptor build() {
            Set<Requires> requires = new HashSet<Requires>(this.requires.values());
            Set<Exports> exports = new HashSet<Exports>(this.exports.values());
            Set<Opens> opens = new HashSet<Opens>(this.opens.values());

            // add dependency on java.base
            if (strict && !name.equals("java.base") && !this.requires.containsKey("java.base")) {
                requires.add(new Requires(Collections.singleton(Requires.Modifier.MANDATED), "java.base", null, null));
            }

            Set<Provides> provides = new HashSet<Provides>(this.provides.values());
     ...
     * <p>
     * Two {@code ModuleDescriptor} objects are compared by comparing their module names lexicographically. Where the module names are equal then the module versions are compared.
     * When comparing the module versions then a module descriptor with a version is considered to succeed a module descriptor that does not have a version. If both versions are
     * {@linkplain Version#parse(String) unparseable} then the {@linkplain #rawVersion() raw version strings} are compared lexicographically. Where the module names are equal and
     * the versions are equal (or not present in both), then the set of modifiers are compared. Sets of modifiers are compared by comparing a <em>binary value</em> computed for
     * each set. If a modifier is present in the set then the bit at the position of its ordinal is {@code 1} in the binary value, otherwise {@code 0}. If the two set of modifiers
     * are also equal then the other components of the module descriptors are compared in a manner that is consistent with {@code equals}.
     * </p>
     ...
     * <p>
     * If the given object is not a {@code ModuleDescriptor} then this method returns {@code false}. Two module descriptors are equal if each of their corresponding components is
     * equal.
     * </p>
     * <p>
     * This method satisfies the general contract of the {@link java.lang.Object#equals(Object) Object.equals} method.
    ...
    public boolean equals(Object ob) {
        ...
        return (name.equals(that.name) && modifiers.equals(that.modifiers) && requires.equals(that.requires) && Objects.equals(packages, that.packages)
                ...
                && Objects.equals(rawVersionString, that.rawVersionString) && Objects.equals(mainClass, that.mainClass));
    }
    ...
    private static <M> String toString(Set<M> mods, String what) {
        ...
        for (M mod : mods) {
            ...
            stringBuilder.append(mod.toString().toLowerCase());
        }
        if (first) {
            first = false;
        } else {
    ...
    /**
     * Compares two {@code Object} arrays, within comparable elements, lexicographically.
     * <p>
     * If the two arrays share a common prefix then the lexicographic comparison is the result of comparing two elements of type {@code T} at an index {@code i} within the
     ...
     * {@link #mismatch(Object[], Object[])} for the definition of a common and proper prefix.)
     * <p>
     * A {@code null} array reference is considered lexicographically less than a non-{@code null} array reference. Two {@code null} array references are considered equal. A
     * {@code null} array element is considered lexicographically than a non-{@code null} array element. Two {@code null} array elements are considered equal.
     * <p>
     ...
     * @return the value {@code 0} if the first and second array are equal and contain the same elements in the same order; a value less than {@code 0} if the first array is
